/*
  Script to power the ROVE storefront

  Functions:
  - loadProducts: fetches product data from products.json and populates the grid
  - addToCart: adds items to a cart stored in localStorage
  - updateCartCount: updates the cart count badge
  - checkout: assembles an order payload and calls sendOrderToSupplier
  - sendOrderToSupplier: placeholder function to demonstrate dropshipping integration

  The cart persists between sessions via localStorage. In a real
  implementation you would integrate with a backend or a third‑party
  e‑commerce API (e.g. Stripe, Shopify) and forward order details to
  Wohu Optical or another supplier.
*/
  ...
  try {
    ...
    products.forEach(product => {
      ...
      card.innerHTML = `
        ...
        <div class="product-colors">Color: ${product.colors.join(', ')}</div>
...
function getCart() {
  const raw = localStorage.getItem('rove_cart');
  return raw ? JSON.parse(raw) : [];
}

function saveCart(cart) {
  localStorage.setItem('rove_cart', JSON.stringify(cart));
}
...
function addToCart(product) {
  const cart = getCart();
  cart.push({ sku: product.sku, name: product.name, price: product.price });
  saveCart(cart);
  updateCartCount();
...
function checkout() {
  ...
  const payload = {
    orderId,
    lineItems: cart.map(item => ({ sku: item.sku, quantity: 1 })),
    customer: {
      name: 'Customer Name',
...
function sendOrderToSupplier(payload) {
  /*
    Placeholder implementation for dropshipping integration.
    In a production app, you would call the Wohu Optical API here with
    authentication and map your order payload to their expected format.
    See the Wohu dropshipping agreement for details on required fields
